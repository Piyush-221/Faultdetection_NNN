...
class FabricDefectModelResNet(nn.Module):
    def __init__(self):
        super(FabricDefectModelResNet, self).__init__()
        self.resnet = models.resnet50(pretrained=True)
        for param in self.resnet.parameters():
            param.requires_grad = True  
        self.resnet.fc = nn.Linear(self.resnet.fc.in_features, 5)  # Adjusting final layer for 5 classes
...
model_resnet = FabricDefectModelResNet()
...
train_embeddings, train_labels = extract_embeddings(train_loader, model_resnet)
val_embeddings, val_labels = extract_embeddings(val_loader, model_resnet)
test_embeddings, test_labels = extract_embeddings(test_loader, model_resnet)
