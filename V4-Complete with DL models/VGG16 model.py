...
class FabricDefectModelVGG(nn.Module):
    def __init__(self):
        super(FabricDefectModelVGG, self).__init__()
        self.vgg = models.vgg16(pretrained=True)
        for param in self.vgg.parameters():
            param.requires_grad = True  
        self.vgg.classifier[6] = nn.Linear(self.vgg.classifier[6].in_features, 5)  # Adjusting final layer for 5 classes
...
model_vgg = FabricDefectModelVGG()
...
train_embeddings, train_labels = extract_embeddings(train_loader, model_vgg)
val_embeddings, val_labels = extract_embeddings(val_loader, model_vgg)
test_embeddings, test_labels = extract_embeddings(test_loader, model_vgg)
