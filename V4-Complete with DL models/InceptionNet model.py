...
# InceptionNet with Fine-Tuning
class FabricDefectModelInception(nn.Module):
    def __init__(self):
        super(FabricDefectModelInception, self).__init__()
        self.inception = models.inception_v3(pretrained=True)
        for param in self.inception.parameters():
            param.requires_grad = True  
        self.inception.AuxLogits.fc = nn.Linear(self.inception.AuxLogits.fc.in_features, 5)  # Adjusting final layer for 5 classes
        self.inception.fc = nn.Linear(self.inception.fc.in_features, 5)  # Adjusting final layer for 5 classes
...
model_inception = FabricDefectModelInception()
...
train_embeddings, train_labels = extract_embeddings(train_loader, model_inception)
val_embeddings, val_labels = extract_embeddings(val_loader, model_inception)
test_embeddings, test_labels = extract_embeddings(test_loader, model_inception)
